...
public class DataProducer {
	...
	public static void main(String[] args) throws Exception {
		...
		properties.put("serializer.class", "com.iot.app.kafka.util.DataEncoder");
		//generate event
		Producer<String, EventData> producer = new Producer<String, EventData>(new ProducerConfig(properties));
	 ...
	 * 
	 */
	private void generateIoTEvent(Producer<String, EventData> producer, String topic) throws InterruptedException {
		...
		while (true) {
			...
			for (int i = 0; i < 100; i++) {// create 100 vehicles
				...
				for (int j = 0; j < 5; j++) {// Add 5 events for each vehicle
					...
					EventData event = new EventData(vehicleId, vehicleType, routeId, latitude, longitude, timestamp, speed,fuelLevel);
					eventList.add(event);
				}
			}
			Collections.shuffle(eventList);// shuffle for random events
			for (EventData event : eventList) {
				KeyedMessage<String, EventData> data = new KeyedMessage<String, EventData>(topic, event);
				producer.send(data);
